...
$Status = $_POST["Status"];
$query = "SELECT * FROM tbl_qtn_category WHERE Name = '$Name'";
...
$qx = "INSERT INTO tbl_qtn_category SET Name = '$Name',Status='$Status'";
	$conn->query($qx);
	echo 1;
}
}
...
if($_POST['action'] == 'fetch_record'){
 $id = $_POST['id'];
    $query = "SELECT * FROM tbl_qtn_category WHERE id = '$id'";
    $result = $conn->query($query);
...
$Name = addslashes(trim($_POST["Name"]));
$Status = $_POST["Status"];
$query = "SELECT * FROM tbl_qtn_category WHERE Name = '$Name' AND id != '$id'";
$result = $conn->query($query);
$row_cnt = mysqli_num_rows($result);
if($row_cnt > 0){
  echo 0;
  ...
  $query2 = "UPDATE tbl_qtn_category SET Name = '$Name',Status='$Status' WHERE id = '$id'";
 	$conn->query($query2);
  echo 1;
}
}
  ...
  if($_POST['action'] == 'delete') {
      ...
      $id = $_POST['id'];
      $query = "DELETE FROM tbl_qtn_category WHERE id = '$id'";
      $conn->query($query);
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
        ...
        <tbody>
          <?php 
 $srno = 1;
  $sql = "SELECT * FROM tbl_qtn_category ORDER BY id DESC";
   $rx = $conn->query($sql);
