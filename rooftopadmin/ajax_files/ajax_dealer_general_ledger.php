...
if($_POST['action'] == 'SaveVed'){
    ...
    $sql2 = "SELECT MAX(SrNo) as maxid FROM tbl_dealer_general_ledger WHERE Type='PR'";
    $row2 = getRecord($sql2);
    if($row2['maxid'] == ''){
        $SrNo = 1;
        $Code = "PR".$SrNo;
    ...
    if($id == ''){
    $sql4 = "INSERT INTO tbl_dealer_general_ledger SET SrNo='$SrNo',Code='$Code',UserId='$UserId',AccountName='$VedName',InvoiceNo='$InvoiceNo',TotAmt='$TotAmt',TotPaidAmt='$TotPaidAmt',TotBalAmt='$TotBalAmt',PaidAmt='$PaidAmt',BalAmt='$BalAmt',CrDr='cr',Roll=5,Type='PR',CreatedDate='$CreatedDate'";
    $conn->query($sql4);
}
else{
    $sql4 = "UPDATE tbl_dealer_general_ledger SET PaidAmt='$PaidAmt',ModifiedDate='$CreatedDate' WHERE id='$id'";
    $conn->query($sql4);
}
    echo 1;
}
...
if($_POST['action'] == 'getInvoiceDetails'){
    ...
    $row = getRecord($sql);
    $sql2 = "SELECT SUM(PaidAmt) As TotPaidAmt FROM tbl_dealer_general_ledger WHERE InvoiceNo='$id' AND Type='PR'";
    $row2 = getRecord($sql2);
    $TotAmt = $row['Amount'];
    ...
    echo json_encode(array('TotAmt'=>$TotAmt,'TotPaidAmt'=>$TotPaidAmt,'TotBalAmt'=>$TotBalAmt));
     ...
     if ($_POST['action'] == 'getCustCommissionDetails') {
        ...
        $sql2 = "SELECT Amount FROM tbl_dealer_general_ledger WHERE DealerId='$dealerid' AND UserId='$id' AND Type='DINV'";
        $row2 = getRecord($sql2);
        $sql = "SELECT SUM(Amount) AS PaidAmt FROM tbl_dealer_general_ledger WHERE DealerId='$dealerid' AND UserId='$id' AND Type!='DINV' AND CrDr='dr'";
        $row = getRecord($sql);
    ...
    if ($_POST['action'] == 'getCollections') {
        $id = $_POST['uid'];
        $dealerid = $_POST['dealerid'];?>
        <h5>Pay Dealer Commission Transactions</h5>
    ...
    <tbody>
        ...
        $sql = "SELECT * FROM tbl_dealer_general_ledger WHERE UserId='$id' AND DealerId='$dealerid' ORDER BY PaymentDate";
        $rx = $conn->query($sql);
    ...
    if($_POST['action'] == 'SavePaymentReceive'){
        $id = $_POST['id'];
        $DealerId = addslashes(trim($_POST["DealerId"]));
    ...
    if($id==''){
        ...
        if($row2['maxid'] == ''){
         ...
         $sql = "INSERT INTO tbl_dealer_general_ledger SET DealerId='$DealerId',SrNo='$SrNo',Code='$Code',UserId='$CustId',AccountName='$CustName'
    ...
    $conn->query($sql);
        ...
        $sql4 = "UPDATE tbl_dealer_general_ledger SET DealerId='$DealerId',UserId='$CustId',AccountName='$CustName',Amount='$Amount',PaymentDate='$PayDate',PayMode='$PayType',ChequeNo='$ChequeNo',ChqDate='$ChqDate',BankName='$BankName',UpiNo='$UpiNo',
