...
if($_POST['action'] == 'getProdStock'){
  ...
  $sql = "SELECT SUM(Qty) AS AvlQty FROM tbl_rooftop_stocks WHERE BranchId='$BranchId' AND CrDr='cr' AND ProductId='$id'";
  $row = getRecord($sql);

  $sql2 = "SELECT SUM(Qty) AS UseQty FROM tbl_rooftop_stocks WHERE BranchId='$BranchId' AND CrDr='dr' AND ProductId='$id'";
  $row2 = getRecord($sql2);
