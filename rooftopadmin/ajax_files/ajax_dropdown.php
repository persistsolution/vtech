...
<?php 
    $DeptId = $_POST['id'];
        $q = "select * from tbl_rooftop_products WHERE BrandId = '$DeptId' AND Status='1'";
        $r = $conn->query($q);
        while($rw = $r->fetch_assoc())
    {
?>
...
<?php 
    $DeptId = $_POST['id'];
        $q = "select * from tbl_rooftop_stocks WHERE ModelNo = '$DeptId' AND BuyStatus='1'";
        $r = $conn->query($q);
        while($rw = $r->fetch_assoc())
    {
?>
...
<span class="custom-control-label"><?php echo $rw['Name'];?></span>
    ...
    <?php 
        $BranchId = $_POST['id'];
            $q = "SELECT * FROM tbl_users WHERE Status='1' AND Roll=27 AND RooftopBranchId='$BranchId'";
            $r = $conn->query($q);
            while($rw = $r->fetch_assoc())
        {
    ?>
    ...
    $VehicalDate = $_POST['vehdate'];
        $sql12 = "SELECT DISTINCT(VehicalNo) AS VehicalNo FROM tbl_rooftop_stocks WHERE VehicalNo!='' AND CrDr='cr' AND VehicalDate='$VehicalDate'";
        $row12 = getList($sql12);
...
<?php 
    $CatId = $_POST['id'];
        $q = "select * from tbl_rooftop_project_sub_head WHERE UnderBy ='$CatId' AND Status='1'";
        $r = $conn->query($q);
        while($rw = $r->fetch_assoc())
    {
?>
...
<?php } } 
 ...
 if($_POST['action'] == 'getCustListByDealer'){?>
    <option value="0" selected="selected" disabled>Select Customer</option>
<?php 
    $DealerId = $_POST['id'];
        $q = "select id,Fname,BeneficiaryId from tbl_users WHERE DealerId = '$DealerId' AND Status='1'";
        $r = $conn->query($q);
        while($rw = $r->fetch_assoc())
    {
?>
                <option value="<?php echo $rw['id']; ?>"><?php echo $rw['Fname']." (".$rw['BeneficiaryId'].")"; ?></option>
<?php } } 
        ?>
