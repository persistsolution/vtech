...
$qx = "INSERT INTO tbl_rooftop_diesel_amount SET TripId = '$TripId',Status='$Status',DriverId = '$DriverId',Amount='$Amount',Narration='$Narration',CrDate='$CrDate',CreatedBy='$user_id',CreatedDate='$CreatedDate'";
	$conn->query($qx);
	echo 1;
}
...
if($_POST['action'] == 'fetch_record'){
 $id = $_POST['id'];
    $query = "SELECT * FROM tbl_rooftop_diesel_amount WHERE id = '$id'";
...
$ModifiedDate = date('Y-m-d');
  $query2 = "UPDATE tbl_rooftop_diesel_amount SET Status='$Status',Amount='$Amount',Narration='$Narration',CrDate='$CrDate',ModifiedBy='$user_id',ModifiedDate='$ModifiedDate' WHERE id = '$id'";
 	$conn->query($query2);
  echo 1;
}
  ...
  if($_POST['action'] == 'delete') {
      ...
      $id = $_POST['id'];
      $query = "DELETE FROM tbl_rooftop_diesel_amount WHERE id = '$id'";
      $conn->query($query);
       $src = "../../uploads/$Photo";
        unlink($src);
...
if($_POST['action'] == 'deletePhoto'){
    $id = $_POST['id'];
    $Photo = $_POST['Photo'];
        $q = "UPDATE tbl_rooftop_diesel_amount SET Photo='' WHERE id=$id";
        $conn->query($q);
        $src = "../../uploads/$Photo";
        unlink($src);
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
        ...
        <tbody>
          <?php 
 $srno = 1;
  $sql = "SELECT * FROM tbl_rooftop_diesel_amount WHERE TripId='".$_POST['TripId']."' ORDER BY id DESC";
   $rx = $conn->query($sql);
