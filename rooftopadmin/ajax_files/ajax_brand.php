...
$Status = $_POST["Status"];
$query = "SELECT * FROM tbl_brand WHERE Name = '$Name'";
...
$qx = "INSERT INTO tbl_brand SET Name = '$Name',Status='$Status'";
	$conn->query($qx);
	echo 1;
}
}
...
$Name = addslashes(trim($_POST["Name"]));
$Status = $_POST["Status"];
$query = "SELECT * FROM tbl_brand WHERE Name = '$Name' AND id != '$id'";
$result = $conn->query($query);
$row_cnt = mysqli_num_rows($result);
if($row_cnt > 0){
  echo 0;
  ...
  $query2 = "UPDATE tbl_brand SET Name = '$Name',Status='$Status' WHERE id = '$id'";
 	$conn->query($query2);
  echo 1;
}
}
