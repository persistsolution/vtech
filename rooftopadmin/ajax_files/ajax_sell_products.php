...
if($_POST['action'] == 'getProdDetails') {
    ...
    $sql = "SELECT tp.*,tc.Name As Category,tsc.Name As Brand FROM tbl_rooftop_products tp 
 ...
 //    $Price = $row['Price'];
  // echo json_encode(array('Price'=>$Price));
   ...
   if($_POST['action'] == 'getProdDetails2') {
    $id = addslashes(trim($_POST['id']));
    $sql = "SELECT tp.*,tc.Name As Category,tsc.Name As Brand,tp2.Price,tp2.Details FROM tbl_rooftop_stocks tp 
            LEFT JOIN tbl_category tc ON tc.id=tp.CatId 
            LEFT JOIN tbl_rooftop_products tp2 ON tp2.id=tp.ProductId 
...
echo "You Cant add stock more than ".$Qty;
 ...
 // if(!empty($_POST["Qty"])) {
 //  $productByCode = $db_handle->runQuery("SELECT * FROM tbl_rooftop_products WHERE Code='" . $_POST["Code"] . "'");
   ...
   <table class="table table-striped table-bordered">
    <thead>
        ...
        <tbody>
            <?php 
            $i=1;
            $sql2 = "SELECT * FROM tbl_rooftop_stocks WHERE ModelNo='".$ModelNo."' AND BuyStatus=0";
            $row2 = getList($sql2);
            foreach($row2 as $result){
          ?>
            <tr>
