...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
 ...
 $srno = 1;
  $sql = "SELECT * FROM tbl_rooftop_products WHERE Status='1' AND Roll!=1 AND ProdSpec=1 ORDER BY id DESC";
   $rx = $conn->query($sql);
  while($nx = $rx->fetch_assoc()){
  		$sql2 = "SELECT * FROM tbl_rooftop_product_specification WHERE ProdId='".$nx['id']."'";
  		if($AcDc!=''){
  		   $sql2.=" AND AcDc='$AcDc'";
  		}
  		if($Surface!=''){
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
    ...
    $PumpOutletSize = $_POST['PumpOutletSize'];
 $srno = 1;
 $sql = "SELECT tp.id,tp.ProductName,tp.Unit,tps.Qty FROM tbl_rooftop_product_specification tps 
         INNER JOIN tbl_rooftop_products tp ON tps.ProdId=tp.id WHERE tp.Roll!=1 AND tps.Qty>0 AND tp.ProdSpec=1";
        if($AcDc!=''){
