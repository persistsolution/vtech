...
$query = "SELECT * FROM tbl_rooftop_issues WHERE Name = '$Name'";
...
$qx = "INSERT INTO tbl_rooftop_issues SET Name = '$Name',Status='$Status',Photo='$Photo',Phone = '$Phone',Phone2='$Phone2',EmailId='$EmailId',Address='$Address'";
	$conn->query($qx);
	echo 1;
}
}
...
if($_POST['action'] == 'fetch_record'){
 $id = $_POST['id'];
    $query = "SELECT * FROM tbl_rooftop_issues WHERE id = '$id'";
    $result = $conn->query($query);
...
$query = "SELECT * FROM tbl_rooftop_issues WHERE Name = '$Name' AND id != '$id'";
$result = $conn->query($query);
$row_cnt = mysqli_num_rows($result);
if($row_cnt > 0){
  echo 0;
  ...
  $query2 = "UPDATE tbl_rooftop_issues SET Name = '$Name', Status='$Status',Photo='$Photo',Phone = '$Phone',Phone2='$Phone2',EmailId='$EmailId',Address='$Address' WHERE id = '$id'";
 	$conn->query($query2);
 }
  echo 1;
}
  ...
  if($_POST['action'] == 'delete') {
      ...
      $id = $_POST['id'];
       $query2 = "SELECT Photo FROM tbl_rooftop_issues WHERE id = '$id'";
    $result2 = $conn->query($query2);
    $row2 = $result2->fetch_assoc();
    $Photo = $row2['Photo'];
      $query = "DELETE FROM tbl_rooftop_issues WHERE id = '$id'";
      $conn->query($query);
       $src = "../../uploads/$Photo";
        unlink($src);
...
if($_POST['action'] == 'deletePhoto'){
    $id = $_POST['id'];
    $Photo = $_POST['Photo'];
        $q = "UPDATE tbl_rooftop_issues SET Photo='' WHERE id=$id";
        $conn->query($q);
        $src = "../../uploads/$Photo";
        unlink($src);
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
        ...
        <tbody>
          <?php 
 $srno = 1;
  $sql = "SELECT * FROM tbl_rooftop_issues ORDER BY id DESC";
   $rx = $conn->query($sql);
