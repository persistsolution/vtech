...
if($_GET['id']==''){
     $qx = "INSERT INTO tbl_rooftop_lead_details SET CompId='$CompId',Message='$Message',ClainStatus='$ClainStatus',CreatedDate='$CreatedDate',CreatedBy='$user_id',NextDate='$NextDate',NextTime='$NextTime'";
  $conn->query($qx);

  $sql = "UPDATE tbl_rooftop_leads SET ClainStatus='$ClainStatus',NextDate='$NextDate',NextTime='$NextTime' WHERE id='$CompId'";
  $conn->query($sql);
  ...
  echo "<script>alert('Lead Record Saved Successfully!');window.location.href='view-lead-action.php?id=$CompId';</script>";
    ...
    $query2 = "UPDATE tbl_rooftop_lead_details SET CompId='$CompId',Message='$Message',ClainStatus='$ClainStatus',ModifiedDate='$ModifiedDate',ModifiedBy='$user_id',NextDate='$NextDate',NextTime='$NextTime' WHERE id = '$id'";
  $conn->query($query2);
