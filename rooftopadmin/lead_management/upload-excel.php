...
<body>
 ...
 <?php 
$id = $_GET['id'];
$sql7 = "SELECT * FROM tbl_rooftop_social_media_marketing WHERE id='1'";
$row7 = getRecord($sql7);
...
$CreatedTime = date('h:i a');
                ...
                if (!empty($CellNo) || !empty($CustName) || !empty($Address)) {
                     ...
                     $qx = "INSERT INTO tbl_rooftop_leads SET TicketNo='$TicketNo',CustId='$CustId',CellNo='$CellNo',CustName = '$CustName',Status='1',Address='$Address',DocumentsStatus='',ClainReason = '$ClainReason',ClainStatus='In Progress',CreatedDate='$CreatedDate',CreatedBy='$user_id'";
                    $conn->query($qx);
                    $PostId = mysqli_insert_id($conn);
                    $n = 10;
                    $Code2 = RandomStringGenerator($n); 
                    $Code = $Code2."".$PostId;
                    $sql = "UPDATE tbl_rooftop_leads SET code='$Code' WHERE id='$PostId'";
                    $conn->query($sql);
  ...
  $conn->query($sql);
         ...
         $sql = "DELETE FROM tbl_rooftop_leads WHERE CellNo='CellNo'";
         $conn->query($sql);
?>
<script>
