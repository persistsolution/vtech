...
<?php 
$id = $_GET['id'];
$sql7 = "SELECT * FROM tbl_rooftop_lead_quotation WHERE id='$id'";
$row7 = getRecord($sql7);
if($_GET['id'] == ''){
    $TermsCondition = "<ol>
...
$QtnSubject = "Quotation for Supply , Installation and commissioning of ongrid solar rooftop system with Remote monitoring system.";

$InvoiceDate = date('Y-m-d');
    $sql8 = "SELECT MAX(id) AS MaxId FROM tbl_rooftop_lead_quotation";
$row8 = getRecord($sql8);
$MaxId = $row8['MaxId'] + 1;
$Invoice_No = "00".$MaxId;
...
$query22 = "SELECT count(*) as totrec FROM tbl_rooftop_lead_quotation_order_products WHERE SellId = '$id'";
...
if($_GET["action"]=="deleteprd")
{
  $id = $_GET["id"];
  $bid = $_GET["oid"];
  $sql11 = "DELETE FROM tbl_rooftop_lead_quotation_order_products WHERE id = '$id'";
  $conn->query($sql11);
  ?>
...
if($number > 0)  
            ...
            if(trim($_POST["ProductId"][$i] != ''))  
              {
                ...
                $sql22 = "INSERT INTO tbl_rooftop_lead_quotation_order_products SET SGST='$SGST',CGST='$CGST',IGST='$IGST',UserId='$CustId',SellId='$SellId',ProductName='$ProductName',Purity='$Purity',Weight='$Weight',Price='$Price',Making='$Making',HmCharge='$HmCharge',Qty='$Qty',TotalRate='$TotalRate',ProductId='$ProductId',ModelNo='$ModelNo',SellDate='$InvoiceDate'";
                $conn->query($sql22);
  ...
  $sql = "DELETE FROM tbl_rooftop_lead_quotation_order_products WHERE SellId='$id'";
$conn->query($sql);
   $number = count($_POST["ProductId"]);
if($number > 0)  
      {  
...
<option selected="" value="">Select Customer</option>
 <?php 
    if($Roll == 1 || $Roll == 7){
    $sql12 = "SELECT ts.* FROM tbl_rooftop_leads ts WHERE ts.ClainStatus='Completed'";
   }
    else{
    $sql12 = "SELECT ts.* FROM tbl_rooftop_leads ts WHERE ts.ClainStatus='Completed' AND ts.AllocateId='$user_id'";    
    }
  $row12 = getList($sql12);
  foreach($row12 as $result){
     ?>
