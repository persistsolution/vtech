...
$sql = "SELECT tu.*,tut.Name As User_Type FROM tbl_users tu 
                    LEFT JOIN tbl_user_type tut ON tu.UserType=tut.id WHERE tu.Roll=5 AND tu.LeadCust=0 AND tu.ProjectType='2' AND tu.CoordinatorStatus=1";
 		}
else{
    $sql = "SELECT tu.*,tut.Name As User_Type FROM tbl_users tu LEFT JOIN tbl_user_type tut ON tu.UserType=tut.id WHERE tu.Roll=5 AND tu.CoordinatorId='$user_id' AND tu.ProjectType='2' AND tu.CoordinatorStatus=1"; 
}
