...
$sql = "SELECT tu.* FROM tbl_rooftop_sell tdo 
        LEFT JOIN tbl_users tu ON tdo.CustId=tu.id WHERE tdo.Inst_Dispatcher_Otp_Verify=1 AND tu.StateId='$StateId' AND tu.Roll=5 AND tu.ProjectType=2";
}

else if($DispatchStatus == 'No'){
    $sql3 = "SELECT GROUP_CONCAT(tdo.CustId) AS CustId FROM tbl_rooftop_sell tdo LEFT JOIN tbl_users tu ON tdo.CustId=tu.id WHERE tdo.Inst_Dispatcher_Otp_Verify=1 AND tu.StateId='$StateId' AND tu.Roll=5 AND tu.ProjectType=2";
...
$sql = "SELECT tu.*,tu.Lattitude AS InstLattitude,tu.Longitude AS InstLongitude FROM tbl_users tu WHERE tu.StateId='$StateId' AND tu.Roll=5 AND tu.FieldSurveyDetails=1 AND id NOT IN ($CustId) AND tu.ProjectType=2";
}

else if($InstallStatus == 'Yes'){
$sql = "SELECT tu.*,ti.Lattitude AS InstLattitude,ti.Longitude AS InstLongitude,ti.id AS InstId FROM tbl_rooftop_installations ti 
                    LEFT JOIN tbl_users tu ON ti.CustId=tu.id WHERE tu.StateId='$StateId' AND tu.Roll=5 AND tu.FieldSurveyDetails=1 AND ti.InstallStatus='Yes' AND tu.ProjectType=2";
}
else if($InstallStatus == 'No'){
$sql = "SELECT tu.*,tu.Lattitude AS InstLattitude,tu.Longitude AS InstLongitude FROM tbl_users tu WHERE tu.StateId='$StateId' AND tu.Roll=5 AND tu.FieldSurveyDetails=1 AND tu.InstOtpVerify=0 AND tu.ProjectType=2";
}

else if($PoInspection == 'Yes'){
$sql = "SELECT tu.*,ti.Lattitude AS InstLattitude,ti.Longitude AS InstLongitude,ti.id AS InstId FROM tbl_rooftop_installations ti 
                    LEFT JOIN tbl_users tu ON ti.CustId=tu.id WHERE tu.StateId='$StateId' AND tu.Roll=5 AND tu.FieldSurveyDetails=1 AND ti.PoInspection='Yes' AND tu.ProjectType=2";
}
else if($PoInspection == 'No'){
$sql = "SELECT tu.* FROM tbl_users tu WHERE tu.StateId='$StateId' AND tu.Roll=5 AND tu.FieldSurveyDetails=1 AND tu.InspectionOtpVerify=0 AND tu.ProjectType=2";
}

else{
    $sql = "SELECT tu.*,ti.Lattitude AS InstLattitude,ti.Longitude AS InstLongitude,ti.id AS InstId FROM tbl_rooftop_installations ti 
                    LEFT JOIN tbl_users tu ON ti.CustId=tu.id WHERE tu.StateId='$StateId' AND tu.Roll=5 AND tu.FieldSurveyDetails=1 AND tu.ProjectType=2 ";
...
if($InstallStatus == 'No' || $PoInspection == 'No' || $DispatchStatus == 'Yes' || $DispatchStatus == 'No'){
    $Action = "";
     ...
     $Action = "<a href='add-rooftop-installation.php?id=".$row['InstId']."&StateId=".$StateId."'><i class='lnr lnr-pencil mr-2'></i></a>";
