...
$id = $_GET['id'];
$recid = $_GET['recid'];
$sql7 = "SELECT * FROM tbl_dealer_general_ledger WHERE id='$id'";
$row7 = getRecord($sql7);
if($_GET['id'] == ''){
    $PayDate = date('Y-m-d');
}
...
<label class="form-label"> Dealer<span class="text-danger">*</span></label>
 <select class="select2-demo form-control" name="DealerId" id="DealerId" required>
...
<label class="form-label"> Customer<span class="text-danger">*</span></label>
 <select class="select2-demo form-control" name="CustId" id="CustId" required onchange="getRecordDetails(this.value,document.getElementById('DealerId').value)">
<option selected="" value="">Select Customer</option>
 <?php 
  $sql12 = "SELECT id,BeneficiaryId,Fname FROM tbl_users WHERE Status='1' AND Roll=5 AND ProjectType=2 AND DealerId='".$row7['DealerId']."' AND DealerId!=0";
  $row12 = getList($sql12);
  foreach($row12 as $result){
     ?>
...
<div class="form-group col-md-12">
   <label class="form-label">Narration</label>
    ...
    $(document).ready(function() {
        ...
        getCollections(CustId, DealerId);
        getPayType(PayType);
        ...
        $('#validation-form').on('submit', function(e) {
            ...
            if ($('#validation-form').valid()) {
                $.ajax({
                    url: "ajax_files/ajax_dealer_general_ledger.php",
                    method: "POST",
        ...
        $(document).on("change", "#DealerId", function(event) {
            var val = this.value;
            var action = "getCustListByDealer";
            $.ajax({
    ...
    function getCollections(uid, dealerid) {
        $.ajax({
            url: "ajax_files/ajax_dealer_general_ledger.php",
            method: "POST",
            data: { action: "getCollections", uid: uid, dealerid: dealerid },
            success: function(data) {
    ...
    function getRecordDetails(uid, dealerid) {
        $.ajax({
            url: "ajax_files/ajax_dealer_general_ledger.php",
            method: "POST",
            data: { action: "getCustCommissionDetails", uid: uid, dealerid: dealerid },
            success: function(data) {
                var res = JSON.parse(data);
                $('#TotalInvAmt').val(res.TotAmt);
                $('#PaidAmount').val(res.PaidAmt);
                $('#BalanceAmt').val(res.BalAmt);
                getCollections(uid, dealerid);
            }
        });
    }
</script>
