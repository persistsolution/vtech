...
$MainPage="Masters";
$Page = "Project-Sub-Head";
?>
<!DOCTYPE html>
<html lang="en" class="default-style layout-fixed layout-navbar-fixed">
<head>
<title><?php echo $Proj_Title; ?> | Project-Sub-Head</title>
<meta charset="utf-8">
...
<h4 class="font-weight-bold py-3 mb-0">List Of Project Sub Head
...
<span class="font-weight-light">Project Sub Head</span>
</h5>
...
<label class="form-label">Project Head <span class="text-danger">*</span></label>
  <select class="form-control" id="UnderBy" name="UnderBy" required="">
<option selected="" disabled="" value="">Select Project Head</option>
<?php 
        $q = "select * from tbl_rooftop_common_master WHERE Status='1' AND Roll=24";
        $r = $conn->query($q);
...
<div class="clearfix"></div>
 ...
 <div class="form-row">
<div class="form-group col">
<label class="form-label">Sub Head Name <span class="text-danger">*</span></label>
...
<div class="clearfix"></div>
  ...
  <div class="form-row">
<div class="form-group col">
<label class="form-label"> No. of days for Work <span class="text-danger">*</span></label>
...
function product_lists(){
  var action = 'view';
      $.ajax({
  type: "POST",
  url: "../ajax_files/ajax_project_sub_head.php",
   data:{action:action},  
  ...
  function error_toast(){
   ...
   $.growl.error({
      title:    'Error',
      message:  'Project Sub Head Name Already Exists',
   ...
   $.growl.notice({
      title:    'Success',
      message:  'New Project Sub Head Added Successfully!',
      location: isRtl ? 'tl' : 'tr'
    });
  }
function update_toast(){
   ...
   $.growl.notice({
      title:    'Success',
      message:  'Project Sub Head Updated Successfully!',
      location: isRtl ? 'tl' : 'tr'
    });
  }
  ...
  $(document).ready(function() {
    ...
    $('#example').DataTable( {
      ...
      $('#add_button').click(function(){  
           $('.modal-title').html("Add <span class='font-weight-light'>Project Sub Head</span>");  
           $('#action').val("Add");  
           $('#id').val('');
      ...
      $('#UnderBy').attr("selected","selected").val('');
       $('#submit').text('Submit');
 ...
 $.ajax({  
                ...
                success:function(data){  
                    ...
                    $('#Status').val(data.Status).attr("selected",true); 
                     $('#UnderBy').val(data.UnderBy).attr("selected",true);  
                    ...
                    $('#show_photo').html('<div class="ui-feed-icon-container float-left pt-2 mr-3 mb-3"><a href="javascript:void(0)" class="ui-icon ui-feed-icon ion ion-md-close bg-secondary text-white" id="delete_photo"></a><img src="../uploads/'+data.Photo+'" alt="" class="img-fluid ticket-file-img" style="width: 64px;height: 64px;"></div>');
                       ...
                       $('#submit').text("Update");   
                       $('.insert_frm').modal('show');
                         $('.modal-title').html("Update <span class='font-weight-light'>Project Sub Head</span>"); 
 ...
 //alert(id);
   swal({
            title: "Are you sure?",
            text: "Deleted All Records Related this Project Sub Head & You will not be able to recover this Product!",
            type: "warning",
            ...
            if (isConfirm) {
                ...
                success:function(data){
              swal("Deleted!", "Project Sub Head has been deleted.", "success");
              ...
              product_lists();
                ...
                swal("Cancelled", "Project Sub Head is safe :)", "error");
            }
        });
