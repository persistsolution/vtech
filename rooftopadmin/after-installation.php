...
$row2 = getRecord($sql2);
$sql = "SELECT * FROM tbl_rooftop_after_installation_calling_info WHERE CustId='$custid' LIMIT 1";
$rncnt7 = getRow($sql);
$row7 = getRecord($sql);
if($rncnt7 > 0){
  $CustId = $row7['CustId'];
...
$sql = "DELETE FROM tbl_rooftop_after_installation_calling_info WHERE SellId='$id' AND CustId='$CustId'";
$conn->query($sql);
$rncnt = $_POST['Rncnt'];
 ...
 <script type="text/javascript">
   $(document).ready(function() {
         $(document).on("change", "#CustId", function(event) {
            var val = this.value;
            window.location.href="after-installation.php?custid="+val;
