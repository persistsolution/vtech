    ...
    <div class="layout-wrapper layout-2">
        <div class="layout-inner">
            ...
            <div class="layout-container">
                ...
                <?php 
$id = $_GET['id'];
$sql7 = "SELECT * FROM tbl_rooftop_quotation WHERE id='$id'";
$row7 = getRecord($sql7);
if($_GET['id'] == ''){
    $TermsCondition = "<ol>
...
$QtnSubject = "PI for Supply , Installation and commissioning of ongrid solar rooftop system with Remote monitoring system.";

$InvoiceDate = date('Y-m-d');
    $sql8 = "SELECT MAX(id) AS MaxId FROM tbl_rooftop_quotation";
$row8 = getRecord($sql8);
$MaxId = $row8['MaxId'] + 1;
$preyr = date('y')-1;
$curryr = date('y');
...
$query22 = "SELECT count(*) as totrec FROM tbl_rooftop_quotation_order_products WHERE SellId = '$id'";
...
if($_GET["action"]=="deleteprd")
{
  $id = $_GET["id"];
  $bid = $_GET["oid"];
  $sql11 = "DELETE FROM tbl_rooftop_quotation_order_products WHERE id = '$id'";
  $conn->query($sql11);
  ?>
...
$DeliveryNote = addslashes(trim($_POST["DeliveryNote"]));
$DispatchDocNo = addslashes(trim($_POST["DispatchDocNo"]));
$DeliveryDate= addslashes(trim($_POST["DeliveryDate"]));
$DispatchThrough = addslashes(trim($_POST["DispatchThrough"]));
...
$File = $imagepath2 ;
  ...
  $File = $_POST['OldPhoto2'];
}


if($_GET['id']==''){
     $qx = "INSERT INTO tbl_rooftop_quotation SET CompId='$CompId',CustId='$CustId',CellNo='$CellNo',CustName = '$CustName',Status='$Status',Address='$Address',InvoiceNo='$InvoiceNo',InvoiceDate = '$InvoiceDate',QtnSubject='$QtnSubject',Details='$Details',TermsCondition='$TermsCondition',CreatedDate='$CreatedDate',CreatedBy='$user_id',RefEnqNo='$RefEnqNo',KindAttn='$KindAttn',TotalAmt='$TotalAmt',ProdSpec='$ProdSpec',Address2='$Address2',SupplierRef='$SupplierRef',BuyerOrderNo='$BuyerOrderNo',BuyerDate='$BuyerDate',CustomizeAmt='$CustomizeAmt',GstAmt='$GstAmt',SubTotal='$SubTotal',PaymentTerms='$PaymentTerms',Freight='$Freight',Delivery='$Delivery',File='$File',DeliveryNote='$DeliveryNote',DispatchDocNo='$DispatchDocNo',DeliveryDate='$DeliveryDate',DispatchThrough='$DispatchThrough',Destination='$Destination'";
  $conn->query($qx);
...
if($number > 0)  
            ...
            if(trim($_POST["ProductId"][$i] != ''))  
              {
                ...
                $sql22 = "INSERT INTO tbl_rooftop_quotation_order_products SET SGST='$SGST',CGST='$CGST',IGST='$IGST',UserId='$CustId',SellId='$SellId',ProductName='$ProductName',Purity='$Purity',Weight='$Weight',Price='$Price',Making='$Making',HmCharge='$HmCharge',Qty='$Qty',TotalRate='$TotalRate',ProductId='$ProductId',ModelNo='$ModelNo',SellDate='$InvoiceDate',SgstAmt='$SgstAmt'";
                $conn->query($sql22);
      ...
      $Steps = "Generate Performa Invoice (PI)";  
  $sql = "SELECT * FROM tbl_steps WHERE CustId='$CustId' AND SrNo='2'";
  $rncnt = getRow($sql);
  if($rncnt > 0){
      $sql = "UPDATE tbl_steps SET Steps='$Steps' WHERE CustId='$CustId' AND SrNo='2'";
      $conn->query($sql);
  }
  else{
  $sql = "INSERT INTO tbl_steps SET SrNo=2,CustId='$CustId',Steps='$Steps',CreatedDate='$CreatedDate',CreatedTime='$CreatedTime',CustName='$CustName',Address='$Address',Phone='$CellNo',LeadId='0',LeadActId='0',url='view-pi.php'";
  $conn->query($sql);
  }
  ...
  echo "<script>alert('PI Created Successfully!');window.location.href='view-quotation.php';</script>";
    ...
    $query2 = "UPDATE tbl_rooftop_quotation SET CompId='$CompId',CustId='$CustId',CellNo='$CellNo',CustName = '$CustName',Status='$Status',Address='$Address',InvoiceNo='$InvoiceNo',InvoiceDate = '$InvoiceDate',QtnSubject='$QtnSubject',Details='$Details',TermsCondition='$TermsCondition',ModifiedDate='$ModifiedDate',ModifiedBy='$user_id',RefEnqNo='$RefEnqNo',KindAttn='$KindAttn',TotalAmt='$TotalAmt',ProdSpec='$ProdSpec',Address2='$Address2',SupplierRef='$SupplierRef',BuyerOrderNo='$BuyerOrderNo',BuyerDate='$BuyerDate',CustomizeAmt='$CustomizeAmt',GstAmt='$GstAmt',SubTotal='$SubTotal',PaymentTerms='$PaymentTerms',Freight='$Freight',Delivery='$Delivery',File='$File',DeliveryNote='$DeliveryNote',DispatchDocNo='$DispatchDocNo',DeliveryDate='$DeliveryDate',DispatchThrough='$DispatchThrough',Destination='$Destination' WHERE id = '$id'";
  $conn->query($query2);
  $SellId = $id;
  $sql = "DELETE FROM tbl_rooftop_quotation_order_products WHERE SellId='$id'";
$conn->query($sql);
   $number = count($_POST["ProductId"]);
if($number > 0)  
      {  
...
<option selected="" value="">Select Customer</option>
 <?php 
 if($Roll==1 || $Roll==7){
  $sql12 = "SELECT * FROM tbl_users WHERE Status='1' AND Roll=5 AND UnderProdStatus=1 AND SurveyDetails=1 AND FieldSurveyDetails=1 AND ProjectType=2";
   }
   else{
     $sql12 = "SELECT * FROM tbl_users WHERE Status='1' AND Roll=5 AND UnderProdStatus=1 AND SurveyDetails=1 AND FieldSurveyDetails=1 AND ProjectType=2 AND CoordinatorId='$user_id'";
 }
  $row12 = getList($sql12);
  foreach($row12 as $result){
     ?>
...
<input type="hidden" name="OldPhoto2" value="<?php echo $row7['File'];?>" id="OldPhoto2">
<span class="custom-file-label"></span>
</label>
<?php if($row7['File']=='') {} else{?>
  <span id="show_photo2">
<div class="ui-feed-icon-container float-left pt-2 mr-3 mb-3"><a href="javascript:void(0)" class="ui-icon ui-feed-icon ion ion-md-close bg-secondary text-white" id="delete_photo2"></a><a href="../uploads/<?php echo $row7['File'];?>" alt="" target="_new"><?php echo $row7['File'];?></a></div>
</span>
<?php } ?>
</div>
