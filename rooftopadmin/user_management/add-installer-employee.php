...
$MainPage="Installer-Employee";
$Page = "Add-Installer-Employee";
?>
<!DOCTYPE html>
                ...
                <div class="layout-content">
                    ...
                    <div class="container-fluid flex-grow-1 container-p-y">
                        <h4 class="font-weight-bold py-3 mb-0"><?php if($_GET['id']) {?>Edit <?php } else{?> Add
                            <?php } ?> Installer Account</h4>

                        <div class="card mb-4">
                            <div class="card-body">
                                <div id="alert_message"></div>
                                <form id="validation-form" method="post" autocomplete="off" action="../ajax_files/ajax_installer_employee.php" enctype="multipart/form-data">
...
<label class="form-label"> Contractor<span class="text-danger">*</span></label>
 <select class="select2-demo form-control" name="UnderInstallerId" id="UnderInstallerId" required>
<option selected="" value="">Select Contractor</option>
 <?php 
  $sql12 = "SELECT * FROM tbl_users WHERE Status='1' AND Roll=40";
  $row12 = getList($sql12);
  foreach($row12 as $result){
     ?>
  <option <?php if($row7["UnderInstallerId"] == $result['id']) {?> selected <?php } ?> value="<?php echo $result['id'];?>">
    <?php echo $result['Fname']; ?></option>
<?php } ?>
</select>
<div class="clearfix"></div>
                                       ...
                                       <div class="form-group col-md-12">
                                            <label class="form-label">Installer Name <span
...
<br>
                                    <div class="row">
                                        <?php  
                                        $sql33 = "SELECT * FROM tbl_options WHERE id IN(85,86,87,88)";
                                        $row33 = getList($sql33);
...
<div class="form-group col-md-4 showall">
                                            ...
                                            <select class="form-control" name="Roll" id="Roll">
                                                ...
                                                <?php 
                                        $q = "select * from tbl_user_type WHERE Status=1 AND id IN (34,35,36,37) ORDER BY Name";
                                        $r = $conn->query($q);
