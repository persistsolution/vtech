...
$InvoiceDate = addslashes(trim($_POST['InvoiceDate']));
  ...
  $Steps = "Order Delivered Successfully";  
$sql = "SELECT * FROM tbl_steps WHERE CustId='$CustomerId' AND SrNo='5'";
  $rncnt = getRow($sql);
  if($rncnt > 0){
      $sql = "UPDATE tbl_steps SET Steps='$Steps' WHERE CustId='$CustomerId' AND SrNo='5'";
      $conn->query($sql);
  }
  else{
  $sql = "INSERT INTO tbl_steps SET SrNo=5,CustId='$CustomerId',Steps='$Steps',CreatedDate='$CreatedDate',CreatedTime='$CreatedTime',CustName='$CustName',Address='$Address',Phone='$CellNo',LeadId='0',LeadActId='0'";
  $conn->query($sql);
  }
...
<h5>Serial No Products</h5>
 ...
 <input type="hidden" name="ProdType[]" id="ProdType1" value='0'>
       <input type="hidden" name="ProductName[]" id="ProductName1" value='<?php echo $nx['ProductName'];?>'>
        <input type="hidden" name="SerialNo[]" id="SerialNo1" value='<?php echo $nx['SerialNo'];?>'>
 <!-- <input type="hidden" name="ModelNo[]" id="ModelNo1" value="<?php echo $row44['ModelNo'];?>"> -->
  <input type="hidden" name="Qty[]" id="Qty1" value="<?php echo $nx['Qty'];?>">
   <input type="hidden" name="Purity[]" id="Purity1" value="<?php echo $nx['Purity'];?>">
            </tr>
