    ...
    <tr>
       ...
       <!--  <th>Rate</th>
        ...
        <tbody id="dynamic_field" >
            <?php 
                $i=1;
             $sql12 = "SELECT ts.*,tp.ProductName AS Product_Name,tp.ModelNo AS Model_No,tp.Unit FROM tbl_rooftop_stocks ts INNER JOIN tbl_rooftop_products tp ON ts.ProductId=tp.id WHERE ts.CrDr='cr' AND ts.ProdType=0 AND ts.VehicalDate='".$_REQUEST['VehicalDate']."' AND ts.VehicalNo='".$_REQUEST["VehicalNo"]."' GROUP BY ts.ProductId ORDER BY ts.ProductId";
             $rncnt2 = getRow($sql12);
             $row12 = getList($sql12);
  foreach($row12 as $result){
...
<td style="width: 100px;"><input type="number" name="BalQty[]" id="BalQty<?php echo $i;?>" class="form-control" placeholder="e.g.,1" value="<?php echo $BalQty;?>" autocomplete="off" min="1" readonly></td>
<td style="width: 100px;"><input type="number" name="Qty[]" id="Qty<?php echo $i;?>" class="form-control" placeholder="e.g.,1" value="0" autocomplete="off" min="0" oninput="checkQty(<?php echo $i;?>)"><br>
<span style="color:red;font-size: 12px;display:none;" id="showmsg">You Entered Qty Greater Than Stock Qty</span></td>
...
<?php 
    $sql22 = "SELECT * FROM tbl_rooftop_stocks WHERE CrDr='cr' AND ProdType='1' AND SerialNo!='' AND VehicalDate='".$_REQUEST['VehicalDate']."' AND VehicalNo='".$_REQUEST["VehicalNo"]."'";
    $rncnt22 = getRow($sql22);
?>
 <input type="hidden" name="Rncnt2" id="Rncnt2" value="<?php echo $rncnt22;?>">
<div class="form-row"> 
 <label class="form-label" style="font-size: 18px;color: #0dc30d;"> Serial No Products &nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" id="selectAll"> Select All</label> 
<div class="col-lg-12">
     ...
     <tbody>
        <?php 
           $i=1;                              
            ...
            $sql33 = "SELECT * FROM tbl_rooftop_distibute_item_details WHERE ProdType=1 AND SerialNo='".$result['SerialNo']."'";
        ...
        ?>
         ...
         <tr>
            <td><label class="custom-control custom-checkbox">
                    <input type="checkbox" id="Check_Id<?php echo $result['id']; ?>" value="0" class="custom-control-input is-valid checkbox" onclick="featured(<?php echo $result['id']; ?>)">
...
selectAllCheckbox.addEventListener('change', function () {
    ...
    checkboxes.forEach(checkbox => {
        checkbox.checked = isChecked; // Set all checkboxes to match the "Select All" state
    });

    // Update the .chkvalue value
    if (isChecked) {
        $('.chkvalue').val(1); // Set to 1 when "Select All" is checked
    } else {
        $('.chkvalue').val(0); // Set to 0 when "Select All" is unchecked
    }
});
...
checkboxes.forEach(checkbox => {
    checkbox.addEventListener('change', function () {
        ...
        if (!this.checked) {
            ...
            //$('.chkvalue').val(0); // Update .chkvalue to 0 if any checkbox is unchecked
        }

        // Check if all individual checkboxes are checked, and check "Select All" if true
        if ([...checkboxes].every(cb => cb.checked)) {
            selectAllCheckbox.checked = true;
            $('.chkvalue').val(1); // Update .chkvalue to 1 if all are checked
        }
    });
});
