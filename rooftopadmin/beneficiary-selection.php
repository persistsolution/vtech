...
$MainPage="Beneficiary-Selection";
$Page = "Beneficiary-Selection";
...
$row2 = getRecord($sql2);
$sql = "SELECT * FROM tbl_rooftop_beneficiary_selection_calling_info WHERE CustId='$custid' LIMIT 1";
$rncnt7 = getRow($sql);
$row7 = getRecord($sql);
if($rncnt7 > 0){
  $CustId = $row7['CustId'];
...
$sql = "DELETE FROM tbl_rooftop_beneficiary_selection_calling_info WHERE SellId='$id' AND CustId='$CustId'";
$conn->query($sql);
$rncnt = $_POST['Rncnt'];
      ...
      if(is_array($result))
          {
              foreach($result as $row => $value)
              {
               ...
               $sql22 = "INSERT INTO tbl_rooftop_beneficiary_selection_calling_info SET SellId='$sellid',CustId='$CustId',QuesId='$QuesId',Question='$Question',Answer='$Answer',Specify='$Specify',CreatedBy='$user_id',CreatedDate='$CreatedDate',CellNo='$CellNo',CustName='$CustName',Taluka='$Taluka',District='$District',PumpCapacity='$PumpCapacity',Subjects='$Subjects'";
                $conn->query($sql22);

              }
            }

  echo "<script>alert('Record Saved Successfully!');window.location.href='beneficiary-selection.php';</script>";
...
<h4 class="font-weight-bold py-3 mb-0">Beneficiary Selection Calling </h4>
...
<option selected="" value="">Select Customer</option>
 <?php 
  $sql12 = "SELECT * FROM tbl_users tu WHERE tu.Roll=5 AND tu.ProjectType=2";
  $row12 = getList($sql12);
...
<?php  
$sql = "SELECT * FROM tbl_rooftop_common_master WHERE Status=1 AND Roll=25";
$rncnt = getRow($sql);?>
<input type="hidden" name="Rncnt" id="Rncnt" value="<?php echo $rncnt;?>">
<?php 
$row = getList($sql);
foreach($row as $result){
  $sql3 = "SELECT * FROM tbl_rooftop_beneficiary_selection_calling_info WHERE QuesId='".$result["id"]."' AND CustId='$CustId'";
  $rncnt3 = getRow($sql3);
  $row3 = getRecord($sql3);
 ...
 <script type="text/javascript">
   $(document).ready(function() {
         $(document).on("change", "#CustId", function(event) {
            var val = this.value;
            window.location.href="beneficiary-selection.php?custid="+val;
