...
$row2 = getRecord($sql2);
$sql = "SELECT * FROM tbl_rooftop_before_inspection_calling_info WHERE CustId='$custid' LIMIT 1";
$rncnt7 = getRow($sql);
$row7 = getRecord($sql);
if($rncnt7 > 0){
  $CustId = $row7['CustId'];
...
$sql = "DELETE FROM tbl_rooftop_before_inspection_calling_info WHERE SellId='$id' AND CustId='$CustId'";
$conn->query($sql);
$rncnt = $_POST['Rncnt'];
      ...
      if(is_array($result))
          {
              foreach($result as $row => $value)
              {
               ...
               $sql22 = "INSERT INTO tbl_rooftop_before_inspection_calling_info SET SellId='$sellid',CustId='$CustId',QuesId='$QuesId',Question='$Question',Answer='$Answer',Specify='$Specify',CreatedBy='$user_id',CreatedDate='$CreatedDate',CellNo='$CellNo',CustName='$CustName',Taluka='$Taluka',District='$District',PumpCapacity='$PumpCapacity',Subjects='$Subjects'";
                $conn->query($sql22);

              }
            }

  echo "<script>alert('Record Saved Successfully!');window.location.href='before-inspection.php';</script>";
 ...
 <script type="text/javascript">
   $(document).ready(function() {
         $(document).on("change", "#CustId", function(event) {
            var val = this.value;
            window.location.href="before-inspection.php?custid="+val;
