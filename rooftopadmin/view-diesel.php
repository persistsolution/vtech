...
<?php  
  $id = $_GET['id'];
  $sql = "SELECT * FROM tbl_rooftop_trip_details WHERE id='$id'";
  $row = getRecord($sql);
?>
...
<h4 class="font-weight-bold py-3 mb-0">List Of Diesel Amount<br>
...
function product_lists(){
  ...
  url: "ajax_files/ajax_diesel_amount.php",
   data:{action:action,TripId:TripId},  
  ...
  function error_toast(){
   ...
   $.growl.error({
      title:    'Error',
      message:  'Diesel Amount Already Exists',
   ...
   $.growl.success({
      title:    'Success',
      message:  'New Diesel Amount Added Successfully!',
      location: isRtl ? 'tl' : 'tr'
    });
  }
