...
$sql = "SELECT tu.*,tut.Name As User_Type FROM tbl_users tu 
                    LEFT JOIN tbl_user_type tut ON tu.UserType=tut.id WHERE tu.Roll=5 AND tu.LeadCust=0 AND tu.ProjectType='$ProjectType' AND tu.FieldSurveyStatus=1";
 		}
else{
    $sql = "SELECT tu.*,tut.Name As User_Type FROM tbl_users tu LEFT JOIN tbl_user_type tut ON tu.UserType=tut.id WHERE tu.Roll=5 AND tu.FieldSurveyId='$user_id' AND tu.ProjectType='$ProjectType' AND tu.FieldSurveyStatus=1"; 
}
