...
if($Roll==1 || $Roll==7){
$sql = "SELECT tp.* FROM tbl_lead_quotation tp WHERE tp.InvoiceDate='".date('Y-m-d')."'";
 		}
else{
    $sql = "SELECT tp.* FROM tbl_lead_quotation tp 
                    LEFT JOIN tbl_leads tl ON tp.CustId=tl.id WHERE tl.AllocateId='$user_id' AND tp.InvoiceDate='".date('Y-m-d')."'"; 
}
...
while ($row = mysqli_fetch_assoc($empRecords)) {
    ...
    $Action = "<a href='add-lead-quotation.php?id=".$row['id']."'><i class='lnr lnr-pencil mr-2'></i></a>&nbsp;&nbsp;
    	<a onClick='return confirm('Are you sure you want delete this record');' href='".$_SERVER['PHP_SELF']."?id=".$row['id']."&action=delete'><i class='lnr lnr-trash text-danger'></i></a>
        &nbsp;&nbsp;
