...
<body class="body-scroll d-flex flex-column h-100 menu-overlay" data-page="products">
    ...
    <?php 
     ...
     $start_from = ($page - 1) * $results_per_page;  
                        $CatId = $_GET['cat_id'];
                        $SubCatId = $_GET['subid'];
                        $query = "SELECT * FROM products WHERE Status='1'  AND Subscription='1' AND CatId='$CatId' AND SubCatId='$SubCatId' ORDER BY id DESC"; 
...
-ms-box-shadow: 0px 2px 5px rgba(0, 0, 0, 0.05); border-radius: 15px; background: #fff;">
             ...
             <?php } ?>
                    ...
                    <div class="media-body " style="padding-top: 15px; padding-right:3px;">
                        ...
                        <span style="font-weight: 500;" id="MinPrice3<?php echo $result["id"];?>">&#8377; <?php echo number_format($result["MinPrice"],2); ?> </span><br>
...
<a href="subscribe-product.php?id=<?php echo $result["id"];?>" class="btn btn-sm btn-default rounded" style="font-size: 12px;" id="add-cart2104" onclick="addCart(2104);"> subscribe</a>
 <?php } ?>
                                </div>
          ...
          <?php $total_pages = ceil($pagerncnt / $results_per_page); ?>
            ...
            for ($i = 1; $i <= $total_pages; $i++) {
             ...
             ?>
                                ...
                                <?php }; ?>
                                <li class="page-item">
                                    <a class="page-link" href="<?php echo $_SERVER['PHP_SELF']; ?>?<?php if($total_pages>=1){?>page=<?php echo $total_pages;?><?php } else{?>&page=<?php echo $total_pages+1;}?>" aria-label="Next">
                                        <span aria-hidden="true">»</span>
