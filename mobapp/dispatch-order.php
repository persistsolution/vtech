...
$sql = "UPDATE tbl_sell SET CustPhoto='$Photo',Cust_Dispatcher_By='$user_id',Cust_Dispatcher_Date='$CreatedDate' WHERE id='$id'";
$conn->query($sql);
            ...
            if(trim($_POST["CheckId"][$i] != ''))  
              {
                ...
                $SellPrdId = addslashes(trim($_POST['SellPrdId'][$i]));
                 ...
                 if($CheckId == 1){
                $sql22 = "UPDATE tbl_sell_products SET CustDispatch='1',CustDispatchBy='$user_id',CustDispatchDate='$CreatedDate' WHERE id='$SellPrdId'";
                $conn->query($sql22);
  ...
  foreach($row12 as $result12){
    ...
    if($result12['CustDispatch'] == 1){
     ...
     <tr style="<?php echo $bgcolor;?>">
        <td><?php if($result12['CustDispatch'] == 1){} else{?>
