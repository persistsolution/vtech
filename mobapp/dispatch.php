...
<a href="dispatch-order.php" class="btn btn-secondary btn-round">Dispatch</a></span> -->
    ...
    <?php 
            ...
            $sql = "SELECT tdo.*,tu.Fname,tu.Phone,tu.Address FROM tbl_sell tdo 
                    LEFT JOIN tbl_users tu ON tdo.CustId=tu.id WHERE tdo.CustId='$user_id'";
            ...
            while($row = $res->fetch_assoc())
            {
               $sql2 = "SELECT (case when Dispatch='1' then sum(Qty) else '0' end) as dispatchqty,(case when Dispatch='0' then sum(Qty) else '0' end) as balqty FROM `tbl_sell_products` WHERE SellId='".$row['id']."'";
                ...
                $sql22 = "SELECT (case when CustDispatch='1' then sum(Qty) else '0' end) as dispatchqty,(case when CustDispatch='0' then sum(Qty) else '0' end) as balqty FROM `tbl_sell_products` WHERE SellId='".$row['id']."'";
                $row22 = getRecord($sql22);
                $dispatchqty2 = $row22['dispatchqty'];
                $balqty2 = $row22['balqty'];
                $totalqty2 = $dispatchqty2+$balqty2;
             ?>
<div class="card mb-4">

                    <div class="card-body">
                        <h6 style="margin-bottom: 1px;"><?php echo $row['Fname']; ?></h6>
                        <address style="margin-bottom: 1px;">
                            <?php echo $row['Address']; ?>
                        </address>
                        <p style="margin-bottom: 1px;"><strong>Ph.: </strong><?php echo $row['Phone']; ?></p>
                        ...
                        <h3 style="color:red;font-size: 13px;">Employee Dispatch Status:</h3>
                        ...
                        <hr>
                        <h3 style="color:red;font-size: 13px;">Customer Dispatch Status:</h3>
                        <p style="margin-bottom: 1px;"><strong>Total Qty: </strong><?php echo $totalqty2;?> | <strong>Confirm Items : </strong><?php echo $dispatchqty2;?> | <strong>Bal: </strong><?php echo $balqty2;?></p>
                        
                       <a href="dispatch-order.php?id=<?php echo $row['id']; ?>&invno=<?php echo $row['InvoiceNo']; ?>" class="btn btn-success btn-finish">Confirm Items</a>
