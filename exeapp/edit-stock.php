...
$sql8 = "SELECT GROUP_CONCAT(ProductId) AS ProductId FROM tbl_stocks WHERE InvId='$id' AND CrDr='cr'";
$row8 = getRecord($sql8);
$SaveProductId = $row8['ProductId'];

$query22 = "SELECT count(*) as totrec FROM tbl_stocks WHERE InvId = '$id' AND CrDr='cr'";
...
if($_GET["action"]=="deleteacc")
{
  $id = $_GET["id"];
  $bid = $_GET["oid"];
  $sql11 = "DELETE FROM tbl_stocks WHERE id = '$id'";
  $conn->query($sql11);
  ?>
    <script type="text/javascript">
      alert("Deleted Successfully!");
      window.location.href="edit-stock.php?id=<?php echo $bid;?>";
    </script>
<?php } 
?>
...
$Narration = addslashes(trim($_POST['Narration']));
     ...
     $qx = "UPDATE tbl_stock_invoice SET BranchId='$BranchId',TotQty='$TotQty',Status=1,ModifiedBy='$user_id',ModifiedDate='$CreatedDate',DmNo='$DmNo',StockDate='$StockDate',Narration='$Narration' WHERE id='$id'";
  $conn->query($qx);
...
if($number > 0)  
            ...
            if(trim($_POST["Qty"][$i] != '' || $_POST["Qty"][$i] != 0))  
              {
               ...
               $sql = "SELECT * FROM tbl_products WHERE id='$ProdId' ";
                ...
                $sql22 = "INSERT INTO tbl_stocks SET InvId='$InvId',ProductId='$ProdId',ProductName='$ProdName',Qty='$Qty',Status='1',CrDr='cr',Narration='$Narration'";
                $conn->query($sql22);
              }  
   ...
   echo "<script>alert('Product Stock Update Successfully!');window.location.href='view-stocks.php';</script>";
...
<h4 class="font-weight-bold py-3 mb-0">Update Product Stock</h4>
...
<?php 
$i=1;
  $sql12 = "SELECT * FROM tbl_stocks WHERE InvId='$id' AND CrDr='cr'";
  $row12 = getList($sql12);
  foreach($row12 as $result){
     ?>
<input type="hidden" name="ProdId[]" id="ProdId<?php echo $i; ?>" class="form-control" placeholder="" value="<?php echo $result["ProductId"]; ?>" autocomplete="off">
<div class="form-row">
<div class="form-group col-md-6 ">
...
<option selected="" value="">Select Product</option>
 <?php 
  ...
  foreach($row12 as $result2){
     ?>
  <option <?php if($result["ProductId"] == $result2['id']) {?> selected <?php } ?> value="<?php echo $result2['id'];?>">
    <?php echo $result2['ProductName']; ?></option>
<?php } ?>
</select>
</div>
...
<input type="number" name="Qty[]" id="Qty<?php echo $i; ?>" class="form-control txt" placeholder="e.g.,1" value="<?php echo $result["Qty"];?>" autocomplete="off" min="1" oninput="getSubTotal()">
</div>
...
<div class="form-group col-md-6 ">
<label class="form-label">Product Name</label>
 <select class="form-control" name="ProdId[]" id="ProdId<?php echo $row_cnt; ?>" >
<option selected="" value="">Select Product</option>
 <?php 
  $sql12 = "SELECT * FROM tbl_products WHERE Status='1'";
  $row12 = getList($sql12);
