...
if($Qty == $rncnt22){  
 ...
 if($_POST['action'] == 'getSerialProdDetails'){
    ...
    $sql33 = "SELECT * FROM tbl_stocks WHERE CrDr='dr' AND ProdType=1 AND SerialNo='$barcodeno'";
   $rncnt33 = getRow($sql33);
   if($rncnt33 > 0){
    echo 0;
   }
    else{
        $sql22 = "SELECT * FROM tbl_distibute_item_details2 WHERE ProdType='1' AND SerialNo!='' AND SerialNo='$barcodeno'";
        $rncnt22 = getRow($sql22);
        if($rncnt22 > 0){
        $row22 = getRecord($sql22);
        echo json_encode($row22);
    }
    else{
        echo 0;
    }
