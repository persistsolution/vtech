    ...
    <tr>
       ...
       <!--  <th>Rate</th>
        ...
        <tbody id="dynamic_field" >
            <?php 
                $i=1;
             $sql12 = "SELECT ts.*,tp.ProductName AS Product_Name,tp.ModelNo AS Model_No,tp.Unit FROM tbl_stocks ts INNER JOIN tbl_products tp ON ts.ProductId=tp.id WHERE ts.CrDr='cr' AND ts.ProdType=0 AND ts.VehicalDate='".$_REQUEST['VehicalDate']."' AND ts.VehicalNo='".$_REQUEST["VehicalNo"]."' ORDER BY ts.ProductId";
             $rncnt2 = getRow($sql12);
             $row12 = getList($sql12);
  foreach($row12 as $result){
...
<div class="col-lg-12">
     <select class="select2-demo form-control" multiple style="width: 100%" name="SerialProd[]">
                                    ...
                                    ?>
                                        <option value="<?php echo $result['id'];?>" selected><?php echo $result['ProductName']." (".$result['SerialNo'].")";?></option>
                                   <?php } } ?>
...
<div class="form-group col-md-12">
 ...
 function getItems(vehno){
     ...
     window.location.href="distribute-item-store.php?action=search&BranchId="+BranchId+"&StoreInchId="+StoreInchId+"&CreatedDate="+CreatedDate+"&VehicalDate="+VehicalDate+"&VehicalNo="+VehicalNo;
 }
