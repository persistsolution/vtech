...
if($_POST['action'] == 'fetch_record'){
 $id = $_POST['id'];
    $query = "SELECT * FROM tbl_courses WHERE id = '$id'";
  ...
  if($_POST['action'] == 'delete') {
    ...
    $id = $_POST['id'];
    $query2 = "SELECT Photo FROM tbl_courses WHERE id = '$id'";
    $result2 = $conn->query($query2);
    $row2 = $result2->fetch_assoc();
    $Photo = $row2['Photo'];
    $query = "DELETE FROM tbl_courses WHERE id = '$id'";
    $conn->query($query);
    $src = "../../uploads/$Photo";
      unlink($src);
...
if($_POST['action'] == 'deletePhoto'){
    $id = $_POST['id'];
    $Photo = $_POST['Photo'];
        $q = "UPDATE tbl_courses SET Photo='' WHERE id=$id";
        $conn->query($q);
        $src = "../../uploads/$Photo";
        unlink($src);
...
if($_POST['action'] == 'deletePhoto2'){
    $id = $_POST['id'];
    $Photo = $_POST['Photo'];
        $q = "UPDATE tbl_courses SET Photo2='' WHERE id=$id";
        $conn->query($q);
        $src = "../../uploads/$Photo";
        unlink($src);
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
        ...
        <tbody>
          <?php 
 $srno = 1;
  $sql = "SELECT tc.*,tu.Fname FROM tbl_courses tc 
          LEFT JOIN tbl_users tu ON tc.ColgId=tu.id ORDER BY tc.id DESC";
   $rx = $conn->query($sql);
  ...
  ?>
           <tr>
             ...
             <td>
              ...
              <a href='add-courses.php?id=<?php echo $nx['id']; ?>' title="Edit" data-original-title="Edit"><i class="lnr lnr-pencil mr-2"></i></a>
