...
$qx = "INSERT INTO payment_method SET Name = '$Name',Status='$Status'";
	$conn->query($qx);
	echo 1;
}
}
...
$Name = addslashes(trim($_POST["Name"]));
$Status = $_POST["Status"];
$query = "SELECT * FROM payment_method WHERE Name = '$Name' AND id != '$id'";
$result = $conn->query($query);
$row_cnt = mysqli_num_rows($result);
if($row_cnt > 0){
  echo 0;
  ...
  $query2 = "UPDATE payment_method SET Name = '$Name',Status='$Status' WHERE id = '$id'";
 	$conn->query($query2);
  echo 1;
}
}
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
        ...
        <tbody>
          <?php 
 $srno = 1;
  $sql = "SELECT * FROM payment_method ORDER BY id DESC";
   $rx = $conn->query($sql);
