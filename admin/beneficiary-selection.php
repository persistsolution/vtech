...
$row2 = getRecord($sql2);
$sql = "SELECT * FROM tbl_beneficiary_selection_calling_info WHERE CustId='$custid' LIMIT 1";
$rncnt7 = getRow($sql);
$row7 = getRecord($sql);
if($rncnt7 > 0){
  $CustId = $row7['CustId'];
...
$sql = "DELETE FROM tbl_beneficiary_selection_calling_info WHERE SellId='$id' AND CustId='$CustId'";
$conn->query($sql);
$rncnt = $_POST['Rncnt'];
      ...
      if(is_array($result))
          {
              foreach($result as $row => $value)
              {
               ...
               $sql22 = "INSERT INTO tbl_beneficiary_selection_calling_info SET SellId='$sellid',CustId='$CustId',QuesId='$QuesId',Question='$Question',Answer='$Answer',Specify='$Specify',CreatedBy='$user_id',CreatedDate='$CreatedDate',CellNo='$CellNo',CustName='$CustName',Taluka='$Taluka',District='$District',PumpCapacity='$PumpCapacity',Subjects='$Subjects'";
                $conn->query($sql22);
...
<?php  
$sql = "SELECT * FROM tbl_common_master WHERE Status=1 AND Roll=25";
$rncnt = getRow($sql);?>
<input type="hidden" name="Rncnt" id="Rncnt" value="<?php echo $rncnt;?>">
<?php 
$row = getList($sql);
foreach($row as $result){
  $sql3 = "SELECT * FROM tbl_beneficiary_selection_calling_info WHERE QuesId='".$result["id"]."' AND CustId='$CustId'";
  $rncnt3 = getRow($sql3);
  $row3 = getRecord($sql3);
