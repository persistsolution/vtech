...
if($searchValue != ''){
    ...
    if($Roll == 1 || $Roll == 7){
            $sql = "SELECT ts.*,tu.Fname FROM tbl_tasks ts 
            LEFT JOIN tbl_users tu ON ts.CustId=tu.id WHERE ts.Status=1";
        }
        else{
            $sql = "SELECT ts.*,tu.Fname FROM tbl_tasks ts 
            LEFT JOIN tbl_users tu ON ts.CustId=tu.id WHERE ts.ExeId IN ($user_id)";
        }
...
while ($row = mysqli_fetch_assoc($empRecords)) {
       ...
       $sql3 = "SELECT tpf.*,tu.Fname,tu.Lname FROM tbl_task_details tpf LEFT JOIN tbl_users tu ON tpf.CreatedBy=tu.id WHERE tpf.CompId='".$row['id']."' AND tpf.CreatedDate='".date('Y-m-d')."' ORDER BY tpf.id DESC LIMIT 1";
                $rncnt3 = getRow($sql3);
                $row3 = getRecord($sql3);
                if($rncnt3 > 0){
