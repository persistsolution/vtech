                                ...
                                <div class="tab-content">
                                    <div class="tab-pane fade active show" id="tabhome125" role="tabpanel" aria-labelledby="tabhome125-tab">
                                        <h5 style="font-size: 18px;color: #0dc30d;"><strong>Product Details</strong></h5>
                                         <?php 
                $i=1;
              $sql12 = "SELECT ts.*,tp.ProductName AS Product_Name,tp.ModelNo AS Model_No,tp.Unit FROM tbl_distibute_item_details ts INNER JOIN tbl_products tp ON ts.ProductId=tp.id WHERE ts.ProdType=0 AND ts.BranchId='$BranchId' GROUP BY ts.ProductId ORDER BY ts.ProductId";
             $rncnt2 = getRow($sql12);
             $row12 = getList($sql12);
...
<div class="card mb-4" id="bgcolor<?php echo $row['id'];?>">
    ...
    <?php 
        $sql22 = "SELECT * FROM tbl_distibute_item_details WHERE ProdType='1' AND SerialNo!='' AND BranchId='$BranchId'";
