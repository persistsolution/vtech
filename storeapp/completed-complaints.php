...
?>
            ...
            $sql = "SELECT tce.*,tp.ServiceType,tp.CellNo,tc.Name As IssueName,tu.BeneficiaryId,tu.EmailId,tu.ProjectType FROM tbl_complaint_engg_actions tce 
                LEFT JOIN tbl_service_complaint tp ON tp.id=tce.CompId 
                LEFT JOIN tbl_issues tc ON tc.id=tp.Issue 
                LEFT JOIN tbl_users tu ON tu.id=tp.CustId 
                WHERE tce.EnggId='$user_id' AND tp.ServiceType='Maintaince'";
            if($_REQUEST['Village']!=''){
...
<div class="card mb-4" id="bgcolor<?php echo $row['id'];?>">
                    ...
                    <div class="card-body">
                        <h6 style="margin-bottom: 1px;font-size: 15px;"><?php echo $row['CustName']; ?></h6>
                         ...
                         <p style="margin-bottom: 1px;"><strong>(<?php echo $row['BeneficiaryId']; ?>)</strong></p>
                          <p style="margin-bottom: 1px;"><strong>Service Date :</strong> <?php echo date("d/m/Y", strtotime(str_replace('-', '/',$row['ServiceDate']))); ?> </p>
                  ...
                  <?php } ?>
                   ...
                   <p style="margin-bottom: 1px;"><strong>Service Type :</strong> <?php echo $row['ServiceType']; ?> </p>
                    ...
                    <p style="margin-bottom: 1px;"><strong>Project Type :</strong> 
                    <?php if($row["ProjectType"]=='1') {?>
                       Pump Projects 
                    <?php } else { ?> Rooftop Projects <?php } ?></p>

                  
                    <p style="margin-bottom: 1px;"><strong>Specify :</strong> <?php echo $row['Specify']; ?> </p>
                    <p style="margin-bottom: 1px;"><strong>Remark :</strong> <?php echo $row['Remark']; ?> </p>
                  
                 <p style="margin-bottom: 1px;"><strong>Status :</strong> 
                    <?php if($row["ClainStatus"]=='Not Solved') {?>
                    <span style="color:red;font-weight:bold;font-size: 17px;"><?php echo $row['ClainStatus']; ?></span> <?php } else { ?> 
                    <span style="color:#1fe11f;font-weight:bold;font-size: 17px;"><?php echo $row['ClainStatus']; ?> </span> <?php } ?> </p>                        
                    </div>
                </div>
                <?php }  ?>
