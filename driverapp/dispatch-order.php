...
<?php
$id = $_GET['id'];
$sql = "SELECT * FROM tbl_sell WHERE id='$id'";
$row7 = getRecord($sql);
$CellNo = $row7['CellNo'];
...
$sql = "UPDATE tbl_sell SET Photo='$Photo',Inst_Dispatcher_By='$user_id',Inst_Dispatcher_Date='$CreatedDate' WHERE id='$id'";
$conn->query($sql);
            ...
            if(trim($_POST["CheckId"][$i] != ''))  
              {
                ...
                $SellPrdId = addslashes(trim($_POST['SellPrdId'][$i]));
                 ...
                 if($CheckId == 1){
                $sql22 = "UPDATE tbl_sell_products SET Dispatch='1',DispatchBy='$user_id',DispatchDate='$CreatedDate' WHERE id='$SellPrdId'";
                $conn->query($sql22);
...
$otp = rand(1000,9999);
$_SESSION['otp'] = $otp;
echo "<script>window.location.href='dispatch-otp-verify.php?id=$id&phone=$CellNo';</script>";
}
?>
...
<div class="form-row">
 ...
 <!-- <div class="form-group col-md-3 Pump">
                                            ...
                                            <select class="form-control" id="CustId" name="CustId" onchange="getItemLists(this.value)">
<option selected="" disabled="" value="">Select Customer</option>
  <?php 
        $q = "SELECT tu.* FROM tbl_users tu WHERE tu.Roll=5 ORDER BY tu.Fname";
        $r = $conn->query($q);
        while($rw = $r->fetch_assoc())
    {
?>
                <option <?php if($_GET['CustId']==$rw['id']){ ?> selected <?php } ?> value="<?php echo $rw['id']; ?>"><?php echo $rw['Fname']; ?></option>
              <?php } ?>
</select>
...
<div class="col-lg-12">
     <select class="select2-demo form-control" multiple style="width: 100%" name="SerialProd[]">
                                    ...
                                    if($Roll==1 || $Roll==7){ 
                                        ...
                                        $sql22 = "SELECT * FROM tbl_distibute_item_details2 WHERE ProdType='1' AND StoreExeId='$user_id' AND SerialNo!=''";
                                    }
                                        $row22 = getList($sql22);
