...
if($row2 > 0){
    echo json_encode(array('msg'=>"Your Phone No Already Registered With Us",'status'=>'0'));
     ...
     $sql = "SELECT id FROM tbl_users WHERE CustomerId='$DealerCode' AND Roll=42";
    $rncnt = getRow($sql);
    if($rncnt > 0){
        $row = getRecord($sql);
        $DealerId = $row['id'];
...
$query = "SELECT * FROM tbl_users WHERE (Phone = '$Username' OR EmailId='$Username') AND Status=1 AND Roll IN(42)";
 $rncnt = getRow($query);
 $row = getRecord($query);
 if($rncnt > 0){
...
if($getotp == $YourOtp){
    ...
    $sql = "UPDATE tbl_installations SET InstallStatus='Yes',InstallationDate='$InstallationDate' WHERE CustId='$id'";
    $conn->query($sql);
